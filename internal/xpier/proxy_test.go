...
func TestDetectMySQLServer(t *testing.T) {
	homeTemp(t)
	got := detectMySQLServer()
	// Either no mysqld is running (empty) or we resolve a TCP endpoint.
	if got != "" && got != "127.0.0.1:3306" {
		t.Errorf("detectMySQLServer = %q", got)
	}
}
